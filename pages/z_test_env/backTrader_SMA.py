...
def run_bt_SMA(ticker, start_date=datetime(2011, 1, 1), end_date=datetime(2012, 12, 31)):  
    cerebro = bt.Cerebro()
    ...
    print('Final Portfolio Value: %.2f' % (float(cerebro.broker.get_fundvalue()* float(cerebro.broker.get_fundshares())) + float(cerebro.broker.get_cash())), '\n')
    return
