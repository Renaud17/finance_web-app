...
class The_Efficient_Frontier(object):
    def __init__(self, RISKY_ASSETS, mark):
    ...
    def ef_setup(self):
      ...
      # calculate portfolio metrics:
        ...
        for i in range(0, len(self.weights)):
            self.portf_vol.append(
                np.sqrt(np.dot(self.weights[i].T, np.dot(self.cov_mat, self.weights[i]))))
      ...
      # locate points creating efficient frontier:
        ...
        self.portf_rtns_ef = np.linspace(
            self.portf_results_df.returns.min(), self.portf_results_df.returns.max(), self.N_POINTS)
    ...
    def final_plot(self):
        ...
        ax.scatter(
            x=self.max_sharpe_portf.volatility, y=self.max_sharpe_portf.returns, c='black', marker='X', s=175, label='Max Sharpe Ratio'
            )
        ax.scatter(
            x=self.min_vol_portf.volatility, y=self.min_vol_portf.returns, c='black', marker='P', s=175, label='Min Volatility'
            )
        self.portf_results_df.plot(
            kind='scatter', x='volatility', y='returns', c='sharpe_ratio', cmap='RdYlGn', edgecolors='black', ax=ax
            )
        ...
        ax.plot(
            self.portf_vol_ef, self.portf_rtns_ef, 'b--'
            )
...
if __name__ == "__main__":
    ...
    The_Efficient_Frontier(RISKY_ASSETS, mark).final_plot()
