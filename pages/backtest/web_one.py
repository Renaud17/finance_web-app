...
class Web_One(object):
    def __init__(self, ticker):
        ...
        # display /
        st.text(dataset.head()); st.text(dataset.shape)
        ...
        st.text('Total dataset has {} samples, and {} features.'.format(dataset.shape[0], dataset.shape[1]))
        ...
        st.text("Training Accuracy (cross validated): %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
        scores = cross_val_score(regressor, XTest.values, yTest.values, cv=gkcv)
        st.text("Test Accuracy (cross validated): %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
        ...
        st.write(BinaryPredict) ; st.write()
        ...
        plt.title(f'Last 100 Buy and Sell signals - {self.company} ({self.ticker})'); plt.legend(loc='best');
        st.pyplot(fig)
