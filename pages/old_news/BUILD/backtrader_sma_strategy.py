...
def backtrader_sma_strategy_run(tick):
    ...
    # plot results
    # fig, ax = plt.subplots()
    cerebro.plot(iplot=False, use='TkAgg')
...
st.title('BACKTRADER - SMA STRATEGY')

if stock_ticker:      
    backtrader_sma_strategy_run(stock_ticker)

else:
    BT_SmaStrategy_ticker = st.text_input('BT_SmaStrategy_Ticker = :')
    if BT_SmaStrategy_ticker:
        backtrader_sma_strategy_run(BT_SmaStrategy_ticker)