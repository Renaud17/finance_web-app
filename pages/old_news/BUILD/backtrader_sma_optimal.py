...
def bt_sma_strategy_optimization_run(tick):
    # download data
    data = bt.feeds.YahooFinanceData(dataname=tick, fromdate=datetime(2021, 1, 3), todate=datetime(2021, 3, 31))
    # create a Cerebro entity
    cerebro = bt.Cerebro(stdstats = False)
    # set up the backtest
    cerebro.adddata(data)
    cerebro.optstrategy(SmaStrategy, ma_period=range(2, 60))
    cerebro.broker.setcash(1000.0)
    cerebro.run(maxcpus=4)



bt_sma_strategy_optimization_run('FB')